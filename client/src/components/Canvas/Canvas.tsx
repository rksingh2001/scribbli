...
const Canvas = ({ height, width, disable }: { height: number, width: number, disable: boolean }) => {
  ...
  const oldPos: React.MutableRefObject<{posX: number, posY: number} | undefined> = useRef();
  ...
  const draw = (pos: { posX: number, posY: number }, color: string, lineWidth: number) => {
    ...
    if (context) {
      ...
      context.beginPath();
      if (!oldPos || !oldPos.current) {
        oldPos.current = pos;
        return;
      }
      context.moveTo(oldPos.current.posX, oldPos.current.posY);
      context.lineTo(pos.posX, pos.posY);
      oldPos.current.posX = pos.posX;
      oldPos.current.posY = pos.posY;
      context.stroke();
      context.closePath();
  ...
  const colorFloodFill = (x: number, y: number, colorToFill: string) => {
    ...
    const eraserColor: number[] = [0, 0, 0, 0];

    if (compareColors(startColor, eraserColor) && utilitySelected === 'eraser') {
      return;
    }
